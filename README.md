...
$$
\begin{cases}
u_{tt} - \Delta u_t - \Delta_p u = \lambda |u|^{\alpha} + \gamma |\nabla u|^{\beta}, & (x, y) \in \Omega,  t \in  (0, T), \\
u(x, y,0) = u_0(x,y), & (x, y) \in \Omega, \\
u_t(x, y, 0) = u_1(x,y), & (x, y) \in \Omega,
\end{cases}
$$

subject to periodic boundary conditions in both spatial directions

$$
\begin{cases}
u(0, y, t) = u(L_x, y, t), \quad u_t(0, y, t) = u_t(L_x, y, t), \\
u(x, 0, t) = u(x, L_y, t), \quad u_t(x, 0, t) = u_t(x, L_y, t).
\end{cases}
$$
...
where the regularized $p$-Laplacian is defined by
...
$$
    ...
    \qquad
    a(|\nabla u|)
    = \big(|\nabla u|^2 + \epsilon^2\big)^{\frac{p-2}{2}}.
$$

The regularization parameter $\epsilon>0$ prevents degeneracy at points where $\nabla u=0$.
...
$$
\frac{d}{dt}\Big(e^{-|k|^2 t}\widehat{v}\Big)
...
$$
\Omega=[0,8\pi] \times [0,8\pi], \qquad
N_x = N_y = 256, \qquad
\Delta t = 10^{-3}, \qquad
T = 10.
$$
...
$$
\begin{cases}
u_0(x,y)=A\cos\left(\frac{2\pi x}{L_x}\right)\cos\left(\frac{2\pi y}{L_y}\right),\\
u_1(x,y)=\varepsilon,
\end{cases}
$$
